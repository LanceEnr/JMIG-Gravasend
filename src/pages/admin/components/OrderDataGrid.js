...
export default function FullFeaturedCrudGrid(props) {
  ...
  const handleClick = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3001/generateorderDateId"
      );
      if (response.data.id) {
        const newRow = {
          id: response.data.id,
  ...
  const handleSaveConfirmed = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:3001/addOrder", {
        _orderNum: actionId,
        _name: customerName,
        _materialType: product,
        _date: formattedDate,
        _status: status,
        _price: parseInt(price, 10) * parseInt(qty, 10),
        _quantity: qty,
        _orderDet: orderDetails,
      });
  ...
  const deleteRecord = async (id) => {
    try {
      const _orderNum = parseInt(id, 10);
